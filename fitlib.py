...
def calculate_bic_gp (posterior, tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, gp_params, gp_rv, gp_blong, gp_phot, rv_gp_feed, blong_gp_feed, phot_gp_feed, spec_constrained=False, rvnull=False) :
    ...
    maximum_likelihood = lnprob_tr_rv_gp_new(posterior['theta'], posterior['theta_priors'], posterior['labels'], posterior['calib_params'], posterior['rvcalib_params'], posterior['flare_params'], posterior['planet_params'], tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, gp_params, gp_rv, gp_blong, gp_phot, rv_gp_feed, blong_gp_feed, phot_gp_feed, spec_constrained, rvnull=rvnull)
...
def update_star_rotation_gp_params(params, labels, theta, gp_phot, gp_blong, gp_rv, include_rvs=False, shared_Prot=False) :
    ...
    if shared_Prot :
        # Set rotation shared period
        phot_params["period"]  = params["prot"]
        blong_params["period"]  = params["prot"]
    else :
        # Set rotation separate period
        phot_params["period"]  = params["phot_prot"]
        blong_params["period"]  = params["blong_prot"]

    if include_rvs :
        if shared_Prot :
            rv_params["period"]  = params["prot"]
        else :
            rv_params["period"]  = params["rv_prot"]
...
def fit_gp_activity(posterior, lc, bjds, rv, rverr, bbjds, blong, blongerr, binsize = 0.1, rot_period=10.4560, period_lim=(1,500), run_gp_mcmc=False, amp=1e-4, nwalkers=32, niter=300, burnin=50, shared_Prot=False, remove_transits=False, include_rvs=False, plot=False, verbose=False) :
    ...
    decaytime = 450
    fix_decaytime = False
    decaytime_lim = (50,1000)

    smoothfactor = 0.8
    fix_smoothfactor = False
    smoothfactor_lim = (0.25,1.25)

    # Run GP on B-long data with a QP kernel
    gp_blong = gp_lib.star_rotation_gp(bbjds, blong, blongerr, period=rot_period, period_lim=period_lim, amplitude=amplitude, decaytime=decaytime, decaytime_lim=decaytime_lim, fix_decaytime=fix_decaytime, smoothfactor=smoothfactor, smoothfactor_lim=smoothfactor_lim, fix_smoothfactor=fix_smoothfactor, fixpars_before_fit=True, fit_mean=fit_mean, fit_white_noise=fit_white_noise, period_label=r"Prot$_B$ [d]", amplitude_label=r"$\alpha_B$", decaytime_label=r"$l_B$", smoothfactor_label=r"$\beta_B$", mean_label=r"$\mu_b$", white_noise_label=r"$\sigma_B$", output_pairsplot="gpblong_activity_pairsplot.png", run_mcmc=True, amp=amp, nwalkers=nwalkers, niter=niter, burnin=burnin, x_label="BJD", y_label="B$_l$ [G]", plot=plot, verbose=verbose)

    # Run GP on RV residuals with a QP kernel
    gp_rv = gp_lib.star_rotation_gp(bjds, rv, rverr, period=rot_period, period_lim=period_lim, fix_period=False, amplitude=amplitude, decaytime=decaytime, decaytime_lim=decaytime_lim, fix_decaytime=fix_decaytime, smoothfactor=smoothfactor, smoothfactor_lim=smoothfactor_lim, fix_smoothfactor=fix_smoothfactor, fixpars_before_fit=True, fit_mean=fit_mean, fit_white_noise=fit_white_noise, period_label=r"Prot$_v$ [d]", amplitude_label=r"$\alpha_v$", decaytime_label=r"$l_v$", smoothfactor_label=r"$\beta_v$", mean_label=r"$\mu_v$", white_noise_label=r"$\sigma_v$", output_pairsplot="gprv_activity_pairsplot.png", run_mcmc=True, amp=amp, nwalkers=nwalkers, niter=niter, burnin=burnin, x_label="BJD", y_label="RV [m/s]", plot=plot, verbose=verbose)
    ...
    # Run GP on photometry data with a SHO kernel
    gp_phot = gp_lib.star_rotation_gp(bin_tbjd, bin_flux, bin_fluxerr, period=rot_period, period_lim=period_lim, amplitude=amplitude, decaytime=decaytime, decaytime_lim=decaytime_lim, fix_decaytime=fix_decaytime, smoothfactor=smoothfactor, smoothfactor_lim=smoothfactor_lim, fix_smoothfactor=fix_smoothfactor, fixpars_before_fit=True, fit_mean=fit_mean, fit_white_noise=fit_white_noise, period_label=r"Prot$_p$ [d]", amplitude_label=r"$\alpha_p$", decaytime_label=r"$l_p$", smoothfactor_label=r"$\beta_p$", mean_label=r"$\mu_p$", white_noise_label=r"$\sigma_p$", output_pairsplot="gpphot_activity_pairsplot.png", run_mcmc=False, amp=amp, nwalkers=nwalkers, niter=niter, burnin=burnin, x_label="BTJD", y_label="Flux", plot=True, verbose=verbose)

    fix_rv_params = True
    if include_rvs :
        fix_rv_params = False

    params, labels, theta, theta_priors  = set_star_rotation_priors(gp_phot, gp_blong, gp_rv, fix_phot_params=False, fix_blong_params=False, fix_rv_params=fix_rv_params, shared_Prot=shared_Prot, ini_prot=rot_period, period_lim=period_lim, decaytime_lim=decaytime_lim, fix_phot_decaytime=fix_decaytime, fix_spec_decaytime=fix_decaytime, smoothfactor_lim=smoothfactor_lim, fix_phot_smoothfactor=fix_smoothfactor, fix_spec_smoothfactor=fix_smoothfactor)

    if run_gp_mcmc and shared_Prot:

        def gp_log_probability(i_theta, i_gp_phot, i_gp_rv, i_gp_blong, i_params, i_labels, i_theta_priors, include_rvs, shared_Prot):
            
            #gp_lp = i_gp_rv.log_prior() + i_gp_blong.log_prior() + i_gp_phot.log_prior()
            #lp = lnprior(theta)
            
            lp = lnprior(i_theta_priors, i_theta, i_labels)
            #lp = lnprior(i_theta_priors, i_theta, i_labels) + gp_lp
            #lp = gp_lp
            if not np.isfinite(lp) :
                return -np.inf
            
            i_gp_phot, i_gp_blong, i_gp_rv = update_star_rotation_gp_params(i_params, i_labels, i_theta, i_gp_phot, i_gp_blong, i_gp_rv, include_rvs=include_rvs, shared_Prot=shared_Prot)
            
            i_gp_phot.compute(bin_tbjd, yerr=bin_fluxerr)
            i_gp_blong.compute(bbjds, yerr=blongerr)
            
            if include_rvs :
                gp_rv.compute(bjds, yerr=rverr)

            lnlike_phot = i_gp_phot.lnlikelihood(bin_flux, quiet=True)
            lnlike_blong = i_gp_blong.lnlikelihood(blong, quiet=True)
            
            if include_rvs :
                lnlike_rv = i_gp_rv.lnlikelihood(rv, quiet=True)
                return lnlike_phot + lnlike_blong + lnlike_rv  + lp
            else :
                return lnlike_phot + lnlike_blong + lp
        ...
        sampler = emcee.EnsembleSampler(nwalkers, ndim, gp_log_probability, args = [gp_phot, gp_rv, gp_blong, params, labels, theta_priors, include_rvs, shared_Prot])
        ...
        seq = list(zip(*percents))
        values = list(map(func, seq))
        #names = gp_phot.kernel.get_parameter_names()
        ...
        params = gp_lib.update_params(params, labels, mean_params)
        gp_phot, gp_blong, gp_rv  = update_star_rotation_gp_params(params, labels, mean_params, gp_phot, gp_blong, gp_rv, include_rvs=include_rvs, shared_Prot=shared_Prot)

        gp_phot.compute(bin_tbjd, yerr=bin_fluxerr)
        gp_blong.compute(bbjds, yerr=blongerr)
        if include_rvs :
            gp_rv.compute(bjds, yerr=rverr)
        ...
        if shared_Prot :
            ...
            prot_key_blong = prot_key_rv = "prot"
            #cornerlabels = [r"$\mu_p$",r"$\mu_B$ [G]",r"$\mu_v$  [m/s]",r"$\sigma_p$", r"$\sigma_B$ [G]", r"$\sigma_v$  [m/s]", r"$\alpha_p$", r"$\alpha_B$ [G]", r"$\alpha_v$  [m/s]", r"Prot [d]"]

            #cornerlabels = [r"$\mu_p$",r"$\mu_B$ [G]",r"$\mu_v$  [m/s]",r"$\sigma_p$", r"$\sigma_B$ [G]", r"$\sigma_v$  [m/s]", r"$\alpha_p$", r"$\alpha_B$ [G]", r"$\alpha_v$  [m/s]", r"$l_p$ [d]", r"$l_s$ [d]", r"Prot [d]"]

            #cornerlabels = [r"$\mu_p$",r"$\mu_B$ [G]",r"$\mu_v$  [m/s]",r"$\sigma_p$", r"$\sigma_B$ [G]", r"$\sigma_v$  [m/s]", r"$\alpha_p$", r"$\alpha_B$ [G]", r"$\alpha_v$  [m/s]", r"$l_p$ [d]", r"$l_s$ [d]", r"$\beta_p$", r"$\beta_s$", r"Prot [d]"]

            cornerlabels = [r"$\mu_p$",r"$\mu_B$ [G]",r"$\sigma_p$", r"$\sigma_B$ [G]", r"$\alpha_p$", r"$\alpha_B$ [G]", r"$l_p$ [d]", r"$l_s$ [d]", r"$\beta_p$", r"$\beta_s$", r"Prot [d]"]

        else :
            prot_key_blong, prot_key_rv = "blong_prot", "rv_prot"
            cornerlabels = [r"$\mu_p$",r"$\mu_B$ [G]",r"$\mu_v$  [m/s]",r"$\sigma_p$", r"$\sigma_B$ [G]", r"$\sigma_v$  [m/s]", r"$\alpha_p$", r"$\alpha_B$ [G]", r"$\alpha_v$  [m/s]", r"Prot$_p$ [d]", r"Prot$_B$ [d]", r"Prot$_v$ [d]"]
        ...
        corner.corner(sampler.flatchain, truths=mean_params, labels=cornerlabels, quantiles=[0.16, 0.5, 0.84],tick_size=14,labelsize=18,tick_rotate=30)

        plt.savefig("pairsplot_prot.png",bbox_inches='tight')
        #corner.corner(sampler.flatchain, truths=mean_params, labels=param_labels)
        plt.show()

        # Do plots:
        if plot :
            plot_gp_photometry(lc, posterior, gp_phot, bin_flux, binsize)
            plot_gp_blong(bbjds, blong, blongerr, posterior, gp_blong, params[prot_key_blong], phase_plot=True)
            if include_rvs :
                plot_gp_blong(bjds, rv, rverr, posterior, gp_rv, params[prot_key_rv], phase_plot=False, ylabel="RV [m/s]")
    else :
        mean_params = theta
        min_params = max_params = err_params = np.zeros_like(theta)
        if plot :
            plot_gp_photometry(lc, posterior, gp_phot, bin_flux, binsize)

    gp_fit_params = {}
    gp_fit_params["param_labels"] = labels
    gp_fit_params["min_params"] = min_params
    gp_fit_params["max_params"] = max_params
    gp_fit_params["mean_params"] = mean_params
    gp_fit_params["err_params"] = err_params
    ...
    return gp_rv, gp_blong, gp_phot, rv_gp_feed, blong_gp_feed, phot_gp_feed, gp_fit_params
...
#posterior probability
def lnprob_tr_rv_gpSHO(theta, theta_priors, labels, calib_params, rvcalib_params, flare_params, planet_params, tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, gp_rv, gp_blong, gp_phot, rv_gp_feed, blong_gp_feed, phot_gp_feed):
    
    #gp_lp = gp_rv.log_prior() + gp_blong.log_prior() + gp_phot.log_prior()
    
    #lp = lnprior(theta)
    #lp = lnprior(theta_priors, theta, labels) + gp_lp
    lp = lnprior(theta_priors, theta, labels)
    if not np.isfinite(lp):
        return -np.inf
    
    log_S0_Blong = np.array(theta)[np.array(labels) == "log_S0_Blong"][0]
    log_S0_RV = np.array(theta)[np.array(labels) == "log_S0_RV"][0]
    log_S0_Phot = np.array(theta)[np.array(labels) == "log_S0_Phot"][0]
    prot = np.array(theta)[np.array(labels) == "Prot"][0]

    gp_blong.set_parameter("kernel:log_S0", log_S0_Blong)
    gp_blong.set_parameter("kernel:log_omega0", np.log(2*np.pi/prot))

    gp_phot.set_parameter("kernel:log_S0", log_S0_Phot)
    gp_phot.set_parameter("kernel:log_omega0", np.log(2*np.pi/prot))

    gp_rv.set_parameter("kernel:log_S0", log_S0_RV)
    gp_rv.set_parameter("kernel:log_omega0", np.log(2*np.pi/prot))
    ...
    #prob = lp + gp_blong.log_likelihood(blong_gp_feed["y"])  + gp_rv.log_likelihood(rv_gp_feed["y"]) + gp_phot.log_likelihood(phot_gp_feed["y"]) + lnlikelihood_tr_rv_gp(theta, labels, calib_params, rvcalib_params, flare_params, planet_params, tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, gp_rv, gp_phot, rv_gp_feed, phot_gp_feed)

    prob = lp + gp_blong.log_likelihood(blong_gp_feed["y"]) + gp_phot.log_likelihood(phot_gp_feed["y"])  + lnlikelihood_tr_rv_gp(theta, labels, calib_params, rvcalib_params, flare_params, planet_params, tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, gp_rv, gp_phot, rv_gp_feed, phot_gp_feed)
...
#posterior probability
def lnprob_tr_rv_gp(theta, theta_priors, labels, calib_params, rvcalib_params, flare_params, planet_params, tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, gp_params, gp_rv, gp_blong, gp_phot, rv_gp_feed, blong_gp_feed, phot_gp_feed, shared_Prot=False, rvnull=False):
    ...
    gp_phot, gp_blong, gp_rv = update_star_rotation_gp_params(gp_params, labels, theta, gp_phot, gp_blong, gp_rv, shared_Prot=shared_Prot)
    ...
    #prob = lp + gp_blong.log_likelihood(blong_gp_feed["y"]) + gp_phot.log_likelihood(phot_gp_feed["y"]) + lnlikelihood_tr_rv_gp(theta, labels, calib_params, rvcalib_params, flare_params, planet_params, tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, gp_rv, gp_phot, rv_gp_feed, phot_gp_feed)
    prob = lp + lnlikelihood_tr_rv_gp(theta, labels, calib_params, rvcalib_params, flare_params, planet_params, tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, gp_rv, gp_phot, rv_gp_feed, phot_gp_feed, rvnull=rvnull)
...
def fitTransitsAndRVsWithMCMCAndGPSHO(tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, b_times, blong, blongerr, priors, lc, gp_rv, gp_blong, gp_phot, rv_gp_feed, blong_gp_feed, phot_gp_feed, gp_fit_params, gp_prior_type='Uniform', phot_binsize=0.1, amp=1e-4, nwalkers=32, niter=100, burnin=20, verbose=False, plot=False, samples_filename="default_samples.h5", appendsamples=False, shared_Prot=False, return_samples=True) :
    ...
    mean_prot = gp_fit_params["mean_params"][3]
    err_prot = gp_fit_params["err_params"][3]
    
    # Append GP parameters:
    log_S0_RV = gp_rv.get_parameter("kernel:log_S0")
    log_S0_Blong = gp_blong.get_parameter("kernel:log_S0")
    log_S0_Phot = gp_phot.get_parameter("kernel:log_S0")
    
    gp_params = {}
    gp_params["log_S0_RV"] = log_S0_RV
    gp_params["log_S0_Blong"] = log_S0_Blong
    gp_params["log_S0_Phot"] = log_S0_Phot
    gp_params["Prot"] = mean_prot

    posterior["gp_params"] = gp_params
    
    labels.append("log_S0_RV")
    labels.append("log_S0_Blong")
    labels.append("log_S0_Phot")
    labels.append("Prot")
    
    theta = np.append(theta, log_S0_RV)
    theta = np.append(theta, log_S0_Blong)
    theta = np.append(theta, log_S0_Phot)
    theta = np.append(theta, mean_prot)
    
    theta_priors["log_S0_RV"], theta_priors["log_S0_Blong"], theta_priors["log_S0_Phot"], theta_priors["Prot"] = {}, {}, {}, {}
    
    theta_priors["log_S0_RV"]['type'] = gp_prior_type
    theta_priors["log_S0_Blong"]['type'] = gp_prior_type
    theta_priors["log_S0_Phot"]['type'] = gp_prior_type
    theta_priors["Prot"]['type'] = gp_prior_type

    if gp_prior_type == 'Normal' :
        cen, sig = gp_fit_params["mean_params"][0], gp_fit_params["err_params"][0]
        theta_priors["log_S0_RV"]['object'] = priorslib.normal_parameter(np.array([cen, sig]))

        cen, sig = gp_fit_params["mean_params"][1], gp_fit_params["err_params"][1]
        theta_priors["log_S0_Blong"]['object'] = priorslib.normal_parameter(np.array([cen, sig]))

        cen, sig = gp_fit_params["mean_params"][2], gp_fit_params["err_params"][2]
        theta_priors["log_S0_Phot"]['object'] = priorslib.normal_parameter(np.array([cen, sig]))

        theta_priors["Prot"]['object'] = priorslib.normal_parameter(np.array([mean_prot, err_prot]))
    
    elif gp_prior_type == 'Uniform' :
        cen, sig = gp_fit_params["mean_params"][0], gp_fit_params["err_params"][0]
        theta_priors["log_S0_RV"]['object'] = priorslib.uniform_parameter(np.array([cen-10*sig, cen+10*sig]))
        
        cen, sig = gp_fit_params["mean_params"][1], gp_fit_params["err_params"][1]
        theta_priors["log_S0_Blong"]['object'] = priorslib.uniform_parameter(np.array([cen-10*sig, cen+10*sig]))

        cen, sig = gp_fit_params["mean_params"][2], gp_fit_params["err_params"][2]
        theta_priors["log_S0_Phot"]['object'] = priorslib.uniform_parameter(np.array([cen-10*sig, cen+10*sig]))
        
        theta_priors["Prot"]['object'] = priorslib.uniform_parameter(np.array([mean_prot-10*err_prot, mean_prot+10*err_prot]))
    ...
    # Initialize the sampler
    sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob_tr_rv_gpSHO, args = [theta_priors, labels, calib_params, rvcalib_params, flare_params, planet_params, tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, gp_rv, gp_blong, gp_phot, rv_gp_feed, blong_gp_feed, phot_gp_feed], backend=backend)
    ...
    if verbose :
        ...
        for i in range(len(gp_theta_fit)) :
            if gp_theta_labels[i] == 'Prot' :
                print(gp_theta_labels[i], "=", gp_theta_fit[i], "+", gp_theta_err[i][0], "-", gp_theta_err[i][1])
            else :
                meanvalue = np.exp(gp_theta_fit[i])
                maxerr = np.exp(gp_theta_fit[i] + gp_theta_err[i][1]) - meanvalue
                minerr = meanvalue - np.exp(gp_theta_fit[i] - gp_theta_err[i][0])
                print(gp_theta_labels[i].replace("log_",""), "=", meanvalue , "+", maxerr , "-", minerr)

        print("----------------")
    gp_posterior = "gp_posterior.pars"
    if verbose:
    ...
    log_omega0 = np.log(2*np.pi/gp_params["Prot"])
    # update gp parameters in the gp objects
    gp_rv.set_parameter("kernel:log_S0", gp_params["log_S0_RV"])
    gp_blong.set_parameter("kernel:log_S0", gp_params["log_S0_Blong"])
    gp_phot.set_parameter("kernel:log_S0", gp_params["log_S0_Phot"])
    gp_phot.set_parameter("kernel:log_omega0", log_omega0)
    gp_rv.set_parameter("kernel:log_omega0", log_omega0)
    gp_blong.set_parameter("kernel:log_omega0", log_omega0)
    #------------
    ...
    if plot :
        ...
        plot_gp_blong(b_times, blong, blongerr, posterior, gp_blong, gp_params["Prot"], phase_plot=False)
        ...
        plot_rv_timeseries(planet_params, rvcalib_params, rv_times, red_rvs, red_rverrs, samples=samples, labels=labels, planet_index=0, plot_residuals=True, phasefold=False)
...
def fitTransitsAndRVsWithMCMCAndGP_old(tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, b_times, blong, blongerr, priors, lc, gp_rv, gp_blong, gp_phot, rv_gp_feed, blong_gp_feed, phot_gp_feed, gp_fit_params, gp_prior_type='Uniform', phot_binsize=0.1, shared_Prot=False, period_lim=(1,500), fix_phot_params=False, fix_blong_params=False, fix_rv_params=False, amp=1e-4, nwalkers=32, niter=100, burnin=20, verbose=False, plot=False, samples_filename="default_samples.h5", appendsamples=False, return_samples=True) :
    ...
    gp_params, gp_labels, gp_theta, gp_theta_priors  = set_star_rotation_priors(gp_phot, gp_blong, gp_rv, fix_phot_params=fix_phot_params, fix_blong_params=fix_blong_params, fix_rv_params=fix_rv_params, shared_Prot=shared_Prot, period_lim=period_lim)

    if fix_phot_params :
        gp_phot = gp_lib.star_rotation_gp_freeze_all_params(gp_phot)
    if fix_blong_params :
        gp_blong = gp_lib.star_rotation_gp_freeze_all_params(gp_blong)
    if fix_rv_params :
        gp_rv = gp_lib.star_rotation_gp_freeze_all_params(gp_rv)
    ...
    gp_phot, gp_blong, gp_rv = update_star_rotation_gp_params(gp_params, gp_theta_labels, gp_theta_fit, gp_phot, gp_blong, gp_rv, shared_Prot=shared_Prot)
...
#posterior probability
def lnprob_tr_rv_gp_new(theta, theta_priors, labels, calib_params, rvcalib_params, flare_params, planet_params, tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, gp_params, gp_rv, gp_blong, gp_phot, rv_gp_feed, blong_gp_feed, phot_gp_feed, spec_constrained, rvnull=False):
        
    lp = lnprior(theta_priors, theta, labels)
    if not np.isfinite(lp):
        return -np.inf

    gp_phot, gp_blong, gp_rv = update_star_rotation_gp_params_new(gp_params, labels, theta, gp_phot, gp_blong, gp_rv, spec_constrained=spec_constrained)
...
def update_star_rotation_gp_params_new(params, labels, theta, gp_phot, gp_blong, gp_rv, spec_constrained=False) :
...
def fitTransitsAndRVsWithMCMCAndGP(tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, b_times, blong, blongerr, priors, gp_priors, lc, phot_binsize=0.1, amp=1e-4, nwalkers=32, niter=100, burnin=20, samples_filename="default_samples.h5", appendsamples=False, return_samples=False, verbose=False, plot=False, plot_individual_transits=False, rvlabel="", gp_spec_constrained=False) :
    ...
    # Initialize the sampler
    sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob_tr_rv_gp_new, args = [theta_priors, labels, calib_params, rvcalib_params, flare_params, planet_params, tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, gp_params, deepcopy(gp_rv), deepcopy(gp_blong), deepcopy(gp_phot), rv_gp_feed, blong_gp_feed, phot_gp_feed, gp_spec_constrained], backend=backend)
    ...
    gp_phot, gp_blong, gp_rv = update_star_rotation_gp_params_new(gp_params, gp_theta_labels, gp_theta_fit, gp_phot, gp_blong, gp_rv, spec_constrained=gp_spec_constrained)
