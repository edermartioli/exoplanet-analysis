...
def generate_parameter(values):
    ...
    if len(values)>=5:
        out_dict['object'].set_value(float(values[3]))
    return out_dict


def derive_filename(input_filename, tail):
    """Derive an output filename from an input priors filename, replacing a
    known extension (.pars or .json) by the given tail. Safe for any input
    extension: it never returns the input path itself.
    """
    import os
    base, ext = os.path.splitext(str(input_filename))
    if ext.lower() not in ('.pars', '.json'):
        base = str(input_filename)
    return base + tail


def read_priors_from_lines(lines, calibration=False, rvcalibration=False, flares=False):
    """
      Function to parse priors from a list of lines in .pars format.
      """
...
def read_priors(filename, calibration=False, rvcalibration=False, flares=False):
    """
      Function to read priors from file. Accepts either a .pars file or a
      system parameters JSON file (see the systemlib module), which is
      converted transparently to the same internal priors representation.
      """
    if str(filename).lower().endswith('.json'):
        from exoplanet_analysis import systemlib
        lines = systemlib.system_to_pars_lines(systemlib.load_system(filename))
    else:
    ...
    return read_priors_from_lines(lines, calibration=calibration, rvcalibration=rvcalibration, flares=flares)


def read_flares_params(flare_priors_dict, output_theta_params=False) :
    
    """Extract flare parameters from a flare priors dictionary.

    Parameters
    ----------
    flare_priors_dict
    output_theta_params : bool, optional (default: False)
    """
...
def read_calib_params(calib_priors_dict, output_theta_params = False):
    ...
    """Extract photometric calibration parameters from a calibration priors dictionary.
...
def read_rvcalib_params(rvcalib_priors_dict, output_theta_params = False):
    ...
    """Extract RV calibration parameters from an RV calibration priors dictionary.
...
def init_rvcalib_priors(ndim=1, coefs=None, jitters=None) :
    ...
    for i in range(int(ndim)):
        ...
        if coefs != None :
            out_dict['object'].set_value(float(coefs[i][c]))
        ...
        if jitters != None :
            out_jitter_dict['object'].set_value(float(jitters[i]))
...
def get_theta_from_priors(planet_priors, calib_priors, flare_priors={}, instrum_priors=None, rvcalib_priors=None) :
    ...
    """Build the free-parameter vector (theta), labels and prior objects from planet+calibration priors for a transit fit.

    Parameters
    ----------
    planet_priors
    calib_priors
    flare_priors : dict, optional (default: {})
    instrum_priors : optional (default: None)
    rvcalib_priors : optional (default: None)
    """
...
def save_posterior(output, params, theta_fit, theta_labels, theta_err, calib=False, ncoeff=1, ref_time=0., save_json=True) :

    """Save fitted posterior parameters to a .pars file and a JSON mirror file.
    ...
    theta_err
        Uncertainties of the free parameters as (err_plus, err_minus) pairs.
    calib : bool, optional (default: False)
    ncoeff : int, optional (default: 1)
    ...
    json_params = {}
    if calib :
        json_params["orderOfPolynomial"] = {"value": ncoeff, "error": None, "prior": "FIXED"}
    ...
    for key in params.keys() :
        if key in theta_labels :
            ...
            if key == 'tau' :
                ...
                json_params[key] = {"value": theta_fit[idx]+ref_time, "error": [theta_err[idx][0], theta_err[idx][1]], "prior": "Normal"}
            else :
                outfile.write("{0}\tNormal\t{1:.10f},{2:.10f}\n".format(key, theta_fit[idx], error))
                json_params[key] = {"value": theta_fit[idx], "error": [theta_err[idx][0], theta_err[idx][1]], "prior": "Normal"}
        else :
            if ('_err' not in key) and ('_pdf' not in key) :
                if key == 'tau' :
                    outfile.write("{0}\tFIXED\t{1}\n".format(key,params[key]+ref_time))
                    json_params[key] = {"value": _json_scalar(params[key]+ref_time), "error": None, "prior": "FIXED"}
                else :
                    outfile.write("{0}\tFIXED\t{1}\n".format(key,params[key]))
                    json_params[key] = {"value": _json_scalar(params[key]), "error": None, "prior": "FIXED"}
    ...
    if save_json :
        try :
            ...
            json_output = derive_filename(output, ".json")
            with open(json_output, "w") as jf :
                _json.dump({"_format": "exoplanet_analysis posterior",
                            "_source": str(output),
                            "parameters": json_params}, jf, indent=4, default=_json_scalar)
        except Exception as e :
            import warnings
            warnings.warn("Could not save JSON posterior sidecar: {}".format(e))


def _json_scalar(x) :
    """Convert numpy scalars/arrays to native types for JSON serialization."""
...
def get_theta_from_transit_rv_priors(planet_priors, calib_priors, flare_priors={}, instrum_priors=None, rvcalib_priors=None) :
    ...
    """Build the free-parameter vector (theta), labels and prior objects for a joint transit+RV fit.
...
def get_theta_from_rv_priors(planet_priors, rvcalib_priors) :
    ...
    """Build the free-parameter vector (theta), labels and prior objects for an RV-only fit.
...
def read_exoplanet_rv_params(prior_dict, output_theta_params = False):
    ...
    """Extract the RV-model parameters of all planets from a priors dictionary.
    ...
    planet_param_ids = ['k', 'tc', 'per', 'esinw', 'ecosw', 'rvsys', 'trend', 'quadtrend']

    # Optional transit-geometry and Rossiter-McLaughlin parameters. These are
    # needed to model the RM anomaly from RV data around transit, and are
    # included only when present in the priors file (see the rmlib module).
    planet_optional_param_ids = ['a', 'rp', 'inc', 'u0', 'u1', 'lambda', 'vsini', 'omega_rm', 'ldc']
    ...
    for planet_index in range(int(n_planets)) :
        ...
        for i in range(len(planet_optional_param_ids)):
            tmp_id = "{0}_{1:03d}".format(planet_optional_param_ids[i], planet_index)
            if tmp_id in prior_dict.keys() :
                param_ids.append(tmp_id)
...
def read_exoplanet_transit_rv_params(prior_dict, output_theta_params = False) :
    ...
    """Extract the transit+RV model parameters of all planets from a priors dictionary.
    ...
    planet_param_ids = ['transit', 'k', 'tc', 'per', 'esinw', 'ecosw', 'rvsys', 'trend', 'quadtrend']
    
    planet_transit_param_ids = ['a','rp','b','inc','u0','u1']

    # Rossiter-McLaughlin parameters are optional; they are included only when
    # present in the priors file (see the rmlib module).
    planet_rm_param_ids = ['lambda','vsini','omega_rm','ldc']
    ...
    for planet_index in range(int(n_planets)) :
        ...
        for i in range(len(planet_rm_param_ids)):
            tmp_id = "{0}_{1:03d}".format(planet_rm_param_ids[i], planet_index)
            if tmp_id in prior_dict.keys() :
                param_ids.append(tmp_id)
...
def read_exoplanet_params(prior_dict, planet_index=0, output_theta_params = False):
    ...
    """Extract the parameters of one planet from a priors dictionary.
    ...
    prim_param_ids = ['k', 'tc', 'a', 'per', 'b', 'inc', 'rp', 'esinw', 'ecosw', 'u0', 'u1', 'rvsys', 'trend', 'quadtrend', 'transit']
...
def read_instrument_params(prior_dict, output_theta_params = False, inst_param_labels = ['u0', 'u1', 'rp']):
    ...
    """Extract per-instrument parameters from a priors dictionary.

    Parameters
    ----------
    prior_dict
    output_theta_params : bool, optional (default: False)
    inst_param_labels : list, optional (default: ['u0', 'u1', 'rp'])
    """
    instrument_params = {}
    param_ids = []
...
def read_starrot_gp_params(prior_dict, output_theta_params = False, spec_constrained=False) :
    ...
    """Extract quasi-periodic GP (star rotation) parameters from a priors dictionary.
...
def get_gp_theta_from_priors(gp_priors) :
    ...
    """Build the free-parameter vector (theta) and labels from Gaussian Process priors.
...
def read_phot_starrot_gp_params(prior_dict, output_theta_params = False):
    ...
    """Extract photometric quasi-periodic GP parameters from a priors dictionary.
...
def update_prior_initial_guess_from_posterior(planet_priors_file, planet_posteriors_file, updated_priors_file=None) :
    ...
    """Update the initial guesses in a priors file using values from a posterior file.

    Parameters
    ----------
    planet_priors_file
        Path to the planet priors file (.pars or system .json).
    planet_posteriors_file
    updated_priors_file : optional (default: None)
    """
...
def update_posteriors_file(posteriors_file, keys_to_update=[], types=[], values=[], values_l=[], values_u=[], updated_posteriors_file=None) :
    ...
    """Update selected entries of a posteriors file with new types and values.

    Parameters
    ----------
    posteriors_file
    keys_to_update : list, optional (default: [])
    types : list, optional (default: [])
    values : list, optional (default: [])
    values_l : list, optional (default: [])
    values_u : list, optional (default: [])
    updated_posteriors_file : optional (default: None)
    """
