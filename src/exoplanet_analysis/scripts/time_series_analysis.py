"""
    ...
    time_series_analysis --input=/Volumes/Samsung_T5/Data/SOPHIE/TOI-1736/analysis/e2ds/TOI-1736_sophie_results+drs.txt --planet_posterior=/Volumes/Samsung_T5/Science/TOI-1736/RV+TRANSITS_ANALYSIS/TOI-1736_posterior.pars -pv
    
    time_series_analysis --input=/Volumes/Samsung_T5/Data/SOPHIE/TOI-2141/analysis/e2ds/TOI-2141_sophie_results+drs.txt --planet_posterior=/Volumes/Samsung_T5/Science/TOI-2141/RV+TRANSITS_ANALYSIS/TOI-2141_posterior.pars
    """
...
def main() :
    ...
    try:
        ...
        if e.code == 0 or e.code is None :
            raise
        print("Error: check usage with time_series_analysis -h "); sys.exit(1);
    ...
    tbl = ascii.read(options.input, data_start=1)

    bjd = np.array(tbl['bjd'])
    vrad, svrad = np.array(tbl['vrad'])*1000, np.array(tbl['svrad'])*1000
    biss, bisserr = np.array(tbl['biss'])*1000, np.array(tbl['bisserr'])*1000
    fwhm, fwhmerr = np.array(tbl['fwhm']), np.array(tbl['fwhmerr'])
    sindex, sindexerr = np.array(tbl['sindex']), np.array(tbl['sindexerr'])
    halpha, halphaerr = np.array(tbl['H-alpha']), np.array(tbl['H-alphaerr'])
    ...
    ### RVs ###
    ###########
    axs[0].set_ylabel(r"$\Delta$RV [m/s]", fontsize=14)
    ...
    ### FWHM ###
    ###########
    axs[1].set_ylabel(r"FWHM [km/s]", fontsize=14)
    ...
    ### BIS ###
    ###########
    axs[2].set_ylabel(r"BIS [m/s]", fontsize=14)
    ...
    ### s-index ###
    ###########
    axs[3].set_ylabel(r"S$_{\rm MW}$", fontsize=14)
    ...
    ### h-alpha ###
    ###########
    axs[4].set_ylabel(r"H$_{\alpha}$", fontsize=14)
    ...
    axs[4].set_xlabel("Time [BJD]", fontsize=14)
