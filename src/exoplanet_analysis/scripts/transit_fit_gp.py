"""
    ...
    transit_fit_gp --input=TOI-3568_tess_lc.fits --epoch_time=1712.5551018 --fold_period=4.4199503 --transit_duration=2.232 --planet_priors=priors/TOI-3568.pars --calib_order=2 --nsteps=1000 --burnin=300 --gp_priors=priors/TOI-3568_gp_priors_phot.pars -vpmal
...
def reduce_lightcurve(time, flux, fluxerr, posterior, planet_index=0) :
    ...
    transit_model *= exoplanetlib.batman_transit_model(time, posterior["planet_params"][planet_index], planet_index=planet_index)
...
def plot_light_curve(time, flux, fluxerr, posterior, planet_index=0, phase_plot=False) :
    ...
    highsamp_transit_model *= exoplanetlib.batman_transit_model(highsamptime, posterior["planet_params"][planet_index], planet_index=planet_index)
...
def main() :
    ...
    try:
        ...
        if e.code == 0 or e.code is None :
            raise
        print("Error: check usage with transit_fit_gp -h "); sys.exit(1);
    ...
    if options.run_mcmc :
        ...
        if options.samples_filename == "" :
            options.samples_filename = priorslib.derive_filename(planet_priors_files[0], "_mcmc_samples.h5")
