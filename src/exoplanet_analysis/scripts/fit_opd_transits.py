"""
    ...
    python -W"ignore" fit_opd_transits.py --object="WASP-78" --input="/Users/eder/Data/SPARC4/wasp-78/reduced/sparc4acs1/20231107/20231107_s4c1_WASP-78b_POLAR_L2_S+N_lc.fits" --planet_priors=/Users/eder/Science/WASP-78/WASP-78.pars --binsize=0.005 --calib_order=1 --target=0 --comps="1,2,3,4,5" --niter=1 --output=20231107_s4c1_WASP-78b_POLAR_L2_S+N_reduced_lc.fits --gp_priors="/Users/eder/ExoplanetAnalysis/priors/gp-priors.pars" -py

    python -W"ignore" fit_opd_transits.py --object="WASP-78" --input="/Users/eder/Data/SPARC4/wasp-78/reduced/sparc4acs2/20231107/20231107_s4c2_WASP-78b_POLAR_L2_S+N_lc.fits" --planet_priors=/Users/eder/Science/WASP-78/WASP-78.pars --binsize=0.005 --calib_order=1 --target=2 --comps="0,1,3,4,5" --niter=1 --output=20231107_s4c2_WASP-78b_POLAR_L2_S+N_reduced_lc.fits --gp_priors="/Users/eder/ExoplanetAnalysis/priors/gp-priors.pars" -vpy

    python -W"ignore" fit_opd_transits.py --object="WASP-108" --input="/Users/eder/Data/OPD/WASP-108_test/reduced/WASP-108_lc.fits" --planet_priors=/Volumes/Samsung_T5/Science/WASP-108/WASP-108.pars --binsize=0.005 --calib_order=1 --target=1 --comps="0,2,3,4,5,6" --output=WASP-108_reduced_lc.fits --niter=1 -vp
    
    
    
    python -W"ignore" fit_opd_transits.py --object="WASP-78" --input="/Users/eder/Science/WASP-78/20231107_s4c1_WASP-78b_POLAR_L2_S+N_lc.fits" --planet_priors=/Users/eder/Science/WASP-78/WASP-78.pars --binsize=0.01 --calib_order=2 --target=0 --comps="1,2,3,4,5" --output=20231107_s4c1_WASP-78b_POLAR_L2_S+N_reduced_lc.fits -py

    python -W"ignore" fit_opd_transits.py --object="WASP-78" --input="/Users/eder/Science/WASP-78/20231107_s4c2_WASP-78b_POLAR_L2_S+N_lc.fits" --planet_priors=/Users/eder/Science/WASP-78/WASP-78.pars --binsize=0.01 --calib_order=2 --target=2 --comps="0,1,3,4" --niter=1 --output=20231107_s4c2_WASP-78b_POLAR_L2_S+N_reduced_lc.fits -vpy

    python -W"ignore" fit_opd_transits.py --object="WASP-78" --input="/Users/eder/Science/WASP-78/20231107_s4c3_WASP-78b_POLAR_L2_S+N_lc.fits" --planet_priors=/Users/eder/Science/WASP-78/WASP-78.pars --binsize=0.01 --calib_order=2 --target=2 --comps="0,1,3,4,5" --niter=1 --output=20231107_s4c3_WASP-78b_POLAR_L2_S+N_reduced_lc.fits -vpy
    
    python -W"ignore" fit_opd_transits.py --object="WASP-78" --input="/Users/eder/Science/WASP-78/20231107_s4c4_WASP-78b_POLAR_L2_S+N_lc.fits" --planet_priors=/Users/eder/Science/WASP-78/WASP-78.pars --binsize=0.01 --calib_order=2 --target=2 --comps="0,1,3,4,5" --niter=1 --output=20231107_s4c4_WASP-78b_POLAR_L2_S+N_reduced_lc.fits -vpy
...
def diff_light_curve(times, fluxes, fluxerrs, posterior, nsig=100, model_time_sampling=0.001, offset_nsig=10, combine_by_median=False, binsize = 0.005, output="", plot_comps=False, plot=False, verbose=False):
    """
    ...
    for j in range(int(posterior["n_planets"])) :
        ...
        if planet_params[planet_transit_id] :
            ...
            obs_transit_models *= exoplanetlib.batman_transit_model(time, planet_params, planet_index=j)
    ...
    if plot :
        ...
        axs0.set_ylabel(r"fluxo relativo (%)", fontsize=20)
        ...
        if plot_comps :
            ...
            axs[1].set_xlabel("tempo (BJD)", fontsize=20)
        else :
            axs0.set_xlabel("tempo (BJD)", fontsize=20)
...
def remove_systematics(times, fluxes, fluxerrs, posterior, calib_polyorder=1, binsize=0.005, plot=False, verbose=False) :
    ...
    for i in range(len(times)) :
        ...
        if plot :
            plt.title("Systematics for comparison star C{:03d}".format(i), fontsize=18)
...
def main() :
    ...
    parser = OptionParser()
    parser.add_option("-i", "--input", dest="input", help='Pattern for input OPD light curve data',type='string',default="")
    parser.add_option("-g", "--catalog", dest="catalog", help='Catalog extension keyword',type='string',default="CATALOG_PHOT_AP010")
    parser.add_option("-t", "--target", dest="target", help='Target index',type='int',default=0)
    parser.add_option("-0", "--t0", dest="t0", help='T0: reference time',type='float',default=0.)
    parser.add_option("-m", "--comps", dest="comps", help='Comparison indices',type='string',default="1,2,3")
    ...
    parser.add_option("-k", "--gp_priors", dest="gp_priors", help='GP priors file name',type='string',default="")
    ...
    parser.add_option("-1", "--output_lc", dest="output_lc", help='Output reduced lightcurve',type='string',default="")
    ...
    try:
        ...
        if e.code == 0 or e.code is None :
            raise
        print("Error: check usage with fit_opd_transits -h "); sys.exit(1);

    if options.verbose:
        if options.input != '' :
            print('Pattern for input light curve data: ', options.input)
    ...
    #times, fluxes, fluxerrs = get_opddata(options.input, target=2, comps=[0,1,3,4], t0=2457000)
    times, fluxes, fluxerrs = get_opddata(options.input, target=options.target, comps=comps, extname=options.catalog, t0=options.t0)

    n_photdatasets = len(times)

    priors = fitlib.read_transit_rv_priors(options.planet_priors, 0, n_photdatasets, planet_index=0, calib_polyorder=options.calib_order,verbose=False)
    ...
    for i in range(options.niter) :
        posterior = fitlib.fitTransits_ols(times, fluxes, fluxerrs, posterior, calib_post_type="Normal", verbose=False, plot=False)
        fluxes, fluxerrs = remove_systematics(times, fluxes, fluxerrs, posterior, calib_polyorder=5, binsize=options.binsize, plot=options.plot)
        times, fluxes, fluxerrs = clean_data(times, fluxes, fluxerrs, posterior, n_sigma_clip=4., plot=options.plot, verbose=options.verbose)
    
    posterior = fitlib.guess_calib(posterior, times, fluxes, prior_type="Normal", plot=False)
    posterior = fitlib.fitTransits_ols(times, fluxes, fluxerrs, posterior, calib_post_type="Normal", verbose=True, plot=False)

    wppos = get_wppos(options.input, target=options.target, comps=comps, extname=options.catalog, verbose=True)
    if len(wppos) :
        if options.gp_priors != "" :
            fluxes, fluxerrs = remove_waveplate_modulation_with_gp(times, fluxes, fluxerrs, wppos, posterior, options.gp_priors, useQPKernel=True, run_mcmc_on_gp=False, combine_by_median=True, plot=options.plot, verbose=options.verbose)
        else :
            fluxes, fluxerrs = remove_waveplate_modulation(times, fluxes, fluxerrs, wppos, posterior, combine_by_median=False, plot=options.plot, verbose=options.verbose)
 
    tbl = diff_light_curve(times, fluxes, fluxerrs, posterior, nsig=3, model_time_sampling=0.001, offset_nsig=10, combine_by_median=False, binsize=options.binsize, output=options.output_lc, plot_comps=options.plot_comps, plot=options.plot, verbose=options.verbose)
