...
from exoplanet_analysis import priorslib, exoplanetlib

import json

tess_lib_dir = os.path.dirname(__file__)
_data_dir = os.path.join(tess_lib_dir, 'data')

# Path to the Claret (2017) TESS limb-darkening coefficients table (packaged data)
claret_coeff_path = os.path.join(_data_dir, 'tess_claret_ldc', 'table25.dat')

# The TESS objects database is a read/write cache. It lives in the user's home
# directory so that the installed package remains read-only. On first use it is
# seeded from the database shipped with the package.
from exoplanet_analysis.config import user_data_dir, ensure_user_data_dir
objects_db = os.path.join(user_data_dir, 'tess_objects.json')
_packaged_objects_db = os.path.join(_data_dir, 'tess_objects.json')
if not os.path.exists(objects_db) :
    ensure_user_data_dir()
    if os.path.exists(_packaged_objects_db) :
        shutil.copyfile(_packaged_objects_db, objects_db)
...
def plot_detrended_timeseries(time, relflux, fluxerr, model, star_name="UNKNOWN") :
    ...
    model
    star_name : str, optional (default: "UNKNOWN")
    """
...
def get_sector_labels(results, sector=0, get_individual_sectors=False, verbose=False) :
    ...
    ----------
    results
    sector : int, optional (default: 0)
    get_individual_sectors : bool, optional (default: False)
...
def retrieve_tess_data_files(object, sector=0, products_wanted_keys = ["DVT","DVM","DVS","DVR"], individual_sectors=False, get_all_dvt_files=True, verbose=False) :
    ...
    # first check if database exists
    """Query MAST (astroquery) and download TESS data products for an object.
    ...
    sector : int, optional (default: 0)
    products_wanted_keys : list, optional (default: ["DVT","DVM","DVS","DVR"])
    individual_sectors : bool, optional (default: False)
    get_all_dvt_files : bool, optional (default: True)
    verbose : bool, optional (default: False)
        Print progress information.
    """
    if os.path.exists(objects_db) :
...
def save_planet_prior(output, teff, ms, rs, tc, per, a, rp, b, inc, u0, u1, ecc=0, w=90, k=0., hasrvdata=False, circular_orbit=True, set_star_density=False, use_impact_parameter=False, planet_index=0, n_planets=1, append=True, all_parameters_fixed=False, use_linear_ld=False) :
    ...
    """Write a planet priors .pars file from stellar and planet parameters.
    ...
    ecc : int, optional (default: 0)
        Orbital eccentricity.
    w : int, optional (default: 90)
        Argument of periastron [deg].
    k : float, optional (default: 0.)
        RV semi-amplitude.
    hasrvdata : bool, optional (default: False)
    circular_orbit : bool, optional (default: True)
    set_star_density : bool, optional (default: False)
    use_impact_parameter : bool, optional (default: False)
    planet_index : int, optional (default: 0)
        Index of the planet (0-based).
    n_planets : int, optional (default: 1)
        Number of planets in the system.
    append : bool, optional (default: True)
    all_parameters_fixed : bool, optional (default: False)
    use_linear_ld : bool, optional (default: False)
    """
    if append :
    ...
    if circular_orbit or all_parameters_fixed :
        ...
        # For a circular orbit both e*sin(w) and e*cos(w) are zero.
        outfile.write("esinw_{:03d}\tFIXED\t0.\n".format(planet_index))
        outfile.write("ecosw_{:03d}\tFIXED\t0.\n".format(planet_index))
    else :
        #outfile.write("ecc_{:03d}\tUniform\t0,1,{:.5f}\n".format(planet_index,ecc))
        #outfile.write("w_{:03d}\tUniform\t0.,360.,{:.3f}\n".format(planet_index,w))
        outfile.write("esinw_{:03d}\tUniform\t0,1,{:.5f}\n".format(planet_index,ecc*np.sin(w * np.pi/180.)))
        outfile.write("ecosw_{:03d}\tUniform\t0,1,{:.5f}\n".format(planet_index,ecc*np.cos(w * np.pi/180.)))

    outfile.close()


def calculate_tcs_within_range(tmin, tmax, tepoch, tperiod) :
    """Calculate tcs within range.
...
def select_transit_windows(selected_dvt_files, tcs, twindow, tdur_days, flux_base_line=1.0, recenter_tcs=True, convert_times_to_bjd=False, plot=False) :
    ...
    twindow
    tdur_days
    flux_base_line : float, optional (default: 1.0)
    recenter_tcs : bool, optional (default: True)
    convert_times_to_bjd : bool, optional (default: False)
...
def load_dvt_files(object, priors_dir=".", transit_window_size = 5, save_priors=True, planet_to_analyze=-1, hasrvdata=False, force_circular_orbit=True, use_star_density=False, use_impact_parameter=False, convert_times_to_bjd=False, use_linear_ld=False, plot=False, verbose=False) :
    ...
    # first check if database exists
    """Download/load TESS DVT products for an object, generate priors files and package the light curves for fitting.

    Parameters
    ----------
    object
        Name of the object (e.g. 'TOI-3568').
    priors_dir : str, optional (default: ".")
    transit_window_size : int, optional (default: 5)
    save_priors : bool, optional (default: True)
    planet_to_analyze : optional (default: -1)
    hasrvdata : bool, optional (default: False)
    force_circular_orbit : bool, optional (default: True)
    use_star_density : bool, optional (default: False)
    use_impact_parameter : bool, optional (default: False)
    convert_times_to_bjd : bool, optional (default: False)
    use_linear_ld : bool, optional (default: False)
    plot : bool, optional (default: False)
...
def redefine_tessranges(planet, tesslc, planet_priors_file, planet_index=0, transit_window_size=5, verbose=False, plot=False) :
    ...
    """Re-select the light curve windows around transits using an updated ephemeris from a priors file.
    ...
    esinw = pl_params['esinw_{0:03d}'.format(planet_index)]
    ecosw = pl_params['ecosw_{0:03d}'.format(planet_index)]
