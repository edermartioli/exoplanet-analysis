...
from exoplanet_analysis import gp_lib

def plot_mosaic_of_lightcurves(times, fluxes, fluxerrs, priors, output='') :
    
    # calcualate flare time tags
    """Plot a mosaic of individual transit light curves with the current model.
...
def fit_continuum(wav, spec, function='polynomial', order=3, nit=5, rej_low=2.0,
    ...
    """Fit the continuum of a spectrum/light curve with iterative sigma-clipping.

    Parameters
    ----------
    wav
    spec
    function : str, optional (default: 'polynomial')
    order : int, optional (default: 3)
    nit : int, optional (default: 5)
    rej_low : float, optional (default: 2.0)
    rej_high : float, optional (default: 2.5)
    grow : int, optional (default: 1)
    med_filt : int, optional (default: 0)
    percentile_low : float, optional (default: 0.)
    percentile_high : float, optional (default: 100.)
    min_points : int, optional (default: 10)
    xlabel : str, optional (default: "")
    ylabel : str, optional (default: "")
    return_polycoeffs : bool, optional (default: False)
    plot_fit : bool, optional (default: True)
    verbose : bool, optional (default: False)
        Print progress information.
    """
    try :
        _rank_warning = np.exceptions.RankWarning  # numpy >= 2.0
    except AttributeError :
        _rank_warning = np.RankWarning  # numpy < 2.0
...
def read_rv_priors(planet_priors_file, n_rvdatasets, verbose=False) :
    ...
    """Read planet priors for an RV-only fit and initialize the fit parameter structures.

    Parameters
    ----------
    planet_priors_file
        Path to the planet priors file (.pars or system .json).
    n_rvdatasets
...
def read_transit_rv_priors(planet_priors_file, n_rvdatasets, n_photdatasets, planet_index=0, calib_polyorder=1, n_instruments=0, instrument_indexes=None, calib_priorsfile="", instrum_priorsfile="",verbose=False) :
    ...
    """Read planet priors for a transit or joint transit+RV fit and initialize the fit parameter structures, including photometric and RV calibration priors.
    ...
    n_instruments : int, optional (default: 0)
    instrument_indexes : optional (default: None)
    calib_priorsfile : str, optional (default: "")
    instrum_priorsfile : str, optional (default: "")
    verbose : bool, optional (default: False)
        Print progress information.
    """
    loc = {}
...
def read_priors(planet_priors_files, n_datasets, flare_priorsfile="", calib_priorsfile="", calib_polyorder=1, n_rvdatasets=0, verbose=False) :
    ...
    n_datasets
        Number of datasets.
    flare_priorsfile : str, optional (default: "")
    calib_priorsfile : str, optional (default: "")
    calib_polyorder : int, optional (default: 1)
        Order of the photometric calibration polynomial.
    n_rvdatasets : int, optional (default: 0)
        Number of RV datasets.
    verbose : bool, optional (default: False)
        Print progress information.
    """
...
def guess_calib_transit_rv(priors, times, fluxes, rv_times, rvs, prior_type="FIXED", remove_transits=True, rv_prior_type="FIXED", plot=False) :
    ...
    """Estimate initial photometric and RV calibration coefficients for a joint transit+RV fit.
    ...
    prior_type : str, optional (default: "FIXED")
        Prior type assigned to the fitted coefficients ('FIXED' or 'Normal').
    remove_transits : bool, optional (default: True)
    rv_prior_type : str, optional (default: "FIXED")
...
def guess_rvcalib(priors, bjds, rvs, prior_type="FIXED", plot=False) :
    ...
    """Estimate initial RV calibration (systemic velocity) coefficients for each RV dataset.
...
def guess_calib(priors, times, fluxes, prior_type="FIXED", remove_flares=True, remove_transits=True, plot=False, multiplanetmodel=False) :
    ...
    """Estimate initial photometric calibration coefficients by fitting polynomials to the out-of-transit data of each light curve.
    ...
    prior_type : str, optional (default: "FIXED")
        Prior type assigned to the fitted coefficients ('FIXED' or 'Normal').
    remove_flares : bool, optional (default: True)
    remove_transits : bool, optional (default: True)
    plot : bool, optional (default: False)
        Show diagnostic plots.
    multiplanetmodel : bool, optional (default: False)
    """
    calib_polyorder = priors["calib_polyorder"]
    calib_priors = priors["calib_priors"]
    calib_params = priors["calib_params"]
...
def calculate_rv_model_new(time, planet_params, include_trend=True, selected_planet_index=-1, include_rm=False, rm_instrum_params=None, rm_instrum_index=None) :
    
    """Calculate rv model new.
    ...
    include_trend : bool, optional (default: True)
    selected_planet_index : optional (default: -1)
    include_rm : bool, optional (default: False)
        Add the Rossiter-McLaughlin anomaly for any planet that has RM
        parameters (lambda, vsini) in its parameter dictionary. See the
        rmlib module.
    rm_instrum_params : dict, optional (default: None)
        Per-instrument (per-bandpass) transit LD parameters, used to tie the RM
        limb darkening to the transit limb darkening of the matching bandpass.
    rm_instrum_index : int, optional (default: None)
        Bandpass index of this RV dataset, used to select the transit LD tied
        to the RM model. See rmlib.resolve_rm_ldc.
    """
    ...
    if selected_planet_index == -1 :
        for planet_index in range(n_planets) :
            ...
            if include_rm and rmlib.has_rm_parameters(planet_params, planet_index) :
                vel += rmlib.rm_rv_anomaly(time, planet_params, planet_index=planet_index, instrum_params=rm_instrum_params, instrum_index=rm_instrum_index)
...
def fitTransitsWithMCMC(times, fluxes, fluxerrs, priors, amp=1e-4, nwalkers=32, niter=100, burnin=20, verbose=False, plot=False, plot_prev_model=False, plot_individual_transits=False, samples_filename="default_samples.h5", transitsplot_output="", pairsplot_output="", best_fit_from_mode=False, appendsamples=False) :
    ...
    """Fit planetary transits to photometry data using MCMC (emcee), and save posterior files, samples and plots.
    ...
    nwalkers : int, optional (default: 32)
        Number of MCMC walkers.
    niter : int, optional (default: 100)
        Number of MCMC steps.
    burnin : int, optional (default: 20)
        Number of MCMC burn-in steps discarded from the chain.
    verbose : bool, optional (default: False)
        Print progress information.
    plot : bool, optional (default: False)
        Show diagnostic plots.
    plot_prev_model : bool, optional (default: False)
    plot_individual_transits : bool, optional (default: False)
    samples_filename : str, optional (default: "default_samples.h5")
        HDF5 file to save the MCMC samples (emcee backend).
    transitsplot_output : str, optional (default: "")
    pairsplot_output : str, optional (default: "")
    best_fit_from_mode : bool, optional (default: False)
        Use the mode instead of the median of the posterior distributions.
    appendsamples : bool, optional (default: False)
        Append to an existing samples file instead of resetting it.
    """
    ...
    if posterior["calib_priorsfile"] != "":
        calib_posterior = priorslib.derive_filename(posterior["calib_priorsfile"], "_posterior.pars")
    else :
    ...
    if instrum_params is not None :
        ...
        if posterior["instrum_priorsfile"] != "":
            instrum_posterior = priorslib.derive_filename(posterior["instrum_priorsfile"], "_posterior.pars")
        else :
    ...
    if posterior["n_flares"] :
        ...
        flare_posterior = priorslib.derive_filename(posterior["flare_priorsfile"], "_posterior.pars")
        if verbose:
    ...
    posterior["planet_posterior_file"] = priorslib.derive_filename(posterior["planet_priors_file"], "_posterior.pars")
    if verbose:
    ...
    systemlib.export_system_posterior(posterior["planet_priors_file"], posterior["planet_posterior_file"], verbose=verbose)
...
def pairs_plot_emcee(samples, labels, calib_params, planet_params, output='', addlabels=True, rvcalib_params={}) :
    ...
    for lab in labels :
        ...
        if lab == 'rhos':
            ...
            newlabels.append(r"$e\sin{\omega}$")
        elif lab == 'ecosw_000':
            newlabels.append(r"$e\cos{\omega}$")
        elif lab == 'ecc_000':
...
def init_walker_ball(theta, labels, theta_priors, nwalkers, amp=1e-4):
    """Initialize the emcee walker ball with a per-parameter spread.

    A single global ``amp`` is a poor choice when the free parameters have very
    different natural scales (e.g. a spin-orbit obliquity lambda that spans
    +/-180 deg alongside a transit time tc known to ~1e-4 d). This helper scales
    the initial spread of each walker coordinate to the width of that
    parameter's prior, so the ball is neither too tight (walkers stuck) nor too
    wide (walkers thrown into bad regions).
    ...
    labels
        Labels of the free parameters.
    theta_priors
        Prior objects keyed by label.
    nwalkers : int
        Number of walkers.
    amp : float, optional (default: 1e-4)
        Fractional amplitude of the ball relative to each prior width (and the
        fallback absolute amplitude when no prior width is available).

    Returns
    -------
    list
        List of ``nwalkers`` starting position vectors.
    """
    theta = np.asarray(theta, dtype=float)
    ndim = len(theta)
    scales = np.full(ndim, amp, dtype=float)
    for i, key in enumerate(labels):
        width = None
        prior = theta_priors.get(key) if hasattr(theta_priors, "get") else None
        if prior is not None:
            ptype = prior.get("type")
            obj = prior.get("object")
            try:
                hypp = getattr(obj, "prior_hypp", None)
                if ptype in ("Uniform", "Jeffreys") and hypp is not None:
                    width = 0.5 * abs(hypp[1] - hypp[0])
                elif ptype in ("Normal", "Normal_positive") and hypp is not None:
                    width = abs(hypp[1])
            except Exception:
                width = None
        if width is not None and np.isfinite(width) and width > 0:
            # spread each coordinate by amp * (prior width), capped to stay well
            # inside the prior so walkers start in a finite-probability region
            scales[i] = amp * width
        else:
            # fall back to a small fraction of the value (or the raw amp)
            scales[i] = amp * max(abs(theta[i]), 1.0)
    return [theta + scales * np.random.randn(ndim) for _ in range(nwalkers)]


def lnprior(theta_priors, theta, labels):
    """Log-prior probability of a parameter vector given the prior distributions.
...
def lnlikelihood(theta, labels, calib_params, flare_params, planet_params, tr_times, fluxes, fluxerrs, instrum_params=None, instrument_indexes=None):
    ...
    planet_params
        Dictionary of planet parameters (internal ids, e.g. per_000).
    tr_times
        List of photometry time arrays [BJD], one per dataset.
    fluxes
        List of flux arrays, one per dataset.
    fluxerrs
        List of flux uncertainty arrays, one per dataset.
    instrum_params : optional (default: None)
    instrument_indexes : optional (default: None)
    """
...
def lnprob(theta, theta_priors, labels, calib_params, flare_params, planet_params, times, fluxes, fluxerrs, instrum_params=None, instrument_indexes=None):
    ...
    #lp = lnprior(theta)
    """Log-posterior probability function used by the MCMC sampler.
...
#posterior probability
def lnprob_tr_rv(theta, theta_priors, labels, calib_params, rvcalib_params, flare_params, planet_params, tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, rvnull=False, include_rm=False, instrum_params=None, instrument_indexes=None, rm_instrument_indexes=None):
    ...
    rvnull : bool, optional (default: False)
    include_rm : bool, optional (default: False)
        Include the Rossiter-McLaughlin anomaly in the RV model (see rmlib).
    """
    lp = lnprior(theta_priors, theta, labels)
    if not np.isfinite(lp):
        return -np.inf

    lnlike = lnlikelihood_tr_rv(theta, labels, calib_params, rvcalib_params, flare_params, planet_params, tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, rvnull=rvnull, instrum_params=instrum_params, instrument_indexes=instrument_indexes, include_rm=include_rm, rm_instrument_indexes=rm_instrument_indexes)
...
#likelihood function
def lnlikelihood_tr_rv(theta, labels, calib_params, rvcalib_params, flare_params, planet_params, tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, rvnull=False, instrum_params=None, instrument_indexes=None, include_rm=False, rm_instrument_indexes=None):
    ...
    for i in range(len(rvs)) :
        ...
        rm_idx = rm_instrument_indexes[i] if (rm_instrument_indexes is not None and i < len(rm_instrument_indexes)) else None
        if rvnull :
            rv_model = np.zeros_like(rv_times[i]) + rvcalib
        else :
            rv_model = calculate_rv_model_new(rv_times[i], planet_params, include_trend=True, include_rm=include_rm, rm_instrum_params=instrum_params, rm_instrum_index=rm_idx) + rvcalib
...
def plot_priors_model(times, fluxes, fluxerrs, priors) :
    ...
    """Plot the light curve data with the transit model computed from the priors.
...
def errfunc(theta, labels, calib_params, flare_params, planet_params, times, fluxes) :
    ...
    """Residual function used by the least-squares (OLS) fits.
...
def update_calib_priors(calib_priors, calib_params, unc=0.05, prior_type="FIXED") :
    ...
    calib_params
        Dictionary of photometric calibration parameters.
    unc : float, optional (default: 0.05)
    prior_type : str, optional (default: "FIXED")
        Prior type assigned to the fitted coefficients ('FIXED' or 'Normal').
    """
...
def fitTransits_ols(times, fluxes, fluxerrs, priors, flare_post_type="FIXED", flares_unc=0.05, calib_post_type="FIXED", calib_unc=0.05, verbose=False, plot=False) :
    ...
    """Least-squares (OLS) fit of planetary transits to photometry data.
    ...
    priors
        Priors dictionary (see fitlib/priorslib readers).
    flare_post_type : str, optional (default: "FIXED")
    flares_unc : float, optional (default: 0.05)
    calib_post_type : str, optional (default: "FIXED")
    calib_unc : float, optional (default: 0.05)
    verbose : bool, optional (default: False)
        Print progress information.
    plot : bool, optional (default: False)
        Show diagnostic plots.
...
def reduce_lightcurves(times, fluxes, fluxerrs, calib_params, flare_params, remove_calib=True, remove_flares=True) :
    ...
    calib_params
        Dictionary of photometric calibration parameters.
    flare_params
    remove_calib : bool, optional (default: True)
    remove_flares : bool, optional (default: True)
    """
    n_datasets = len(times)
...
def plot_posterior_multimodel(times, fluxes, fluxerrs, posterior, output='', timelabel='BTJD', plot_prev_model=False) :
    ...
    output : str, optional (default: '')
        Output file path.
    timelabel : str, optional (default: 'BTJD')
        Label of the time axis (for plots).
    plot_prev_model : bool, optional (default: False)
    """
    font = {'size': 24}
...
def plot_posterior_instrument_model(times, fluxes, fluxerrs, posterior, output='', timelabel='BTJD', plot_prev_model=False) :
    """Plot posterior instrument model.
...
def plot_all_transits(times, fluxes, fluxerrs, posterior, plot_prev_model=False, bindata=False, binsize=0.01, output='') :
    ...
    posterior
        Posterior dictionary as returned by the fitting routines.
    plot_prev_model : bool, optional (default: False)
    bindata : bool, optional (default: False)
    binsize : float, optional (default: 0.01)
...
#posterior probability
def lnprob_rv(theta, theta_priors, labels, rvcalib_params, planet_params, rv_times, rvs, rverrs, rvnull=False, include_rm=False):
    ...
    lnlike = lnlikelihood_rv(theta, labels, rvcalib_params, planet_params, rv_times, rvs, rverrs, rvnull=rvnull, include_rm=include_rm)
    prob = lp + lnlike
...
#likelihood function
def lnlikelihood_rv(theta, labels, rvcalib_params, planet_params, rv_times, rvs, rverrs, rvnull=False, include_rm=False):
    ...
    for i in range(len(rvs)) :
        ...
        if rvnull :
            ...
            rv_model = calculate_rv_model_new(rv_times[i], planet_params, include_trend=True, include_rm=include_rm) + rvcalib
        rv_residuals = rvs[i] - rv_model
...
def fitRVsWithMCMC(rv_times, rvs, rverrs, priors, amp=1e-4, nwalkers=32, niter=100, burnin=20, samples_filename="default_samples.h5", appendsamples=False, return_samples=True, rvdatalabels=[], addnparams=0, verbose=False, bin_plot_data=False, plot=False, include_rm=False, scaled_ball=True) :
    
    """Fit a Keplerian orbit model to radial velocity data using MCMC (emcee).
    ...
    return_samples : bool, optional (default: True)
        Return the flattened MCMC samples in the posterior dictionary.
    rvdatalabels : list, optional (default: [])
        Labels of the RV datasets (for plots).
    addnparams : int, optional (default: 0)
    verbose : bool, optional (default: False)
        Print progress information.
    bin_plot_data : bool, optional (default: False)
    plot : bool, optional (default: False)
        Show diagnostic plots.
    include_rm : bool, optional (default: False)
        Include the Rossiter-McLaughlin anomaly in the RV model. Use this to
        fit an RM effect from RV data taken around transit (see rmlib and
        fitRMWithMCMC).
    """
    posterior = deepcopy(priors)
    ...
    if verbose and include_rm and rmlib.has_rm_parameters(planet_params, 0):
        print(rmlib.rm_ldc_report(planet_params, 0))
    ...
    # Initialize the sampler
    sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob_rv, args = [theta_priors, labels, rvcalib_params, planet_params, rv_times, rvs, rverrs, False, include_rm], backend=backend)

    pos = init_walker_ball(theta, labels, theta_priors, nwalkers, amp=amp) if scaled_ball else [theta + amp * np.random.randn(ndim) for i in range(nwalkers)]
    #--------
    ...
    # update flare parameters in output posterior
    posterior["planet_params"] = planet_params
    # expose the best-fit planet parameter arrays so callers can save the
    # posterior (including RM parameters) regardless of the verbose flag
    posterior["planet_theta_fit"] = planet_theta_fit
    posterior["planet_theta_labels"] = planet_theta_labels
    posterior["planet_theta_err"] = planet_theta_err
    ...
    if verbose:
        ...
        planet_posterior = priorslib.derive_filename(posterior["planet_priors_file"], "_posterior.pars")
        if verbose:
        ...
        systemlib.export_system_posterior(posterior["planet_priors_file"], planet_posterior, verbose=verbose)
...
def read_rm_priors(planet_priors_file, n_rvdatasets, verbose=False) :

    """Read planet priors for a Rossiter-McLaughlin (RM) fit from RV data.

    This is a thin convenience wrapper around ``read_rv_priors``: the RM-model
    parameters (lambda, vsini, omega_rm, ldc, plus the transit-geometry
    parameters a, rp, inc) are read automatically when present in the priors
    file. The returned structure is used with ``fitRMWithMCMC``.

    Parameters
    ----------
    planet_priors_file
        Path to the planet priors file (.pars or system .json). It must
        contain, in addition to the orbital parameters, the RM parameters
        ``lambda_{iii}`` and ``vsini_{iii}`` and the geometry parameters
        ``a_{iii}``, ``rp_{iii}`` and ``inc_{iii}`` (see the rmlib module).
    n_rvdatasets
        Number of RV datasets.
    verbose : bool, optional (default: False)
        Print progress information.
    ...
    -------
    dict
        The initialized priors/posterior structure.
    """
    return read_rv_priors(planet_priors_file, n_rvdatasets, verbose=verbose)


def fitRMWithMCMC(rv_times, rvs, rverrs, priors, amp=1e-4, nwalkers=32, niter=100, burnin=20, samples_filename="default_samples.h5", appendsamples=False, return_samples=True, rvdatalabels=[], addnparams=0, verbose=False, bin_plot_data=False, plot=False, scaled_ball=True) :

    """Fit the Rossiter-McLaughlin effect to RV data taken around transit.

    This fits a Keplerian orbit plus the classical Ohta et al. (2005) RM
    anomaly to radial velocities. It reuses the same MCMC machinery as the
    orbit-only RV fit (``fitRVsWithMCMC``); the only difference is that the RM
    anomaly is added to the RV model (``include_rm=True``). The free
    parameters typically include the obliquity ``lambda_{iii}`` and the
    projected rotation velocity ``vsini_{iii}``, with the orbital and
    geometric parameters usually held fixed from a prior transit+RV analysis.
    ...
    rverrs
        List of RV uncertainty arrays, one per dataset.
    priors
        Priors dictionary (see read_rm_priors).
    amp : float, optional (default: 1e-4)
        Amplitude of the initial ball of walkers around the starting point.
    nwalkers : int, optional (default: 32)
        Number of MCMC walkers.
    ...
    -------
    dict
        The posterior structure, including the fitted RM parameters.
    """
    return fitRVsWithMCMC(rv_times, rvs, rverrs, priors, amp=amp, nwalkers=nwalkers,
                          niter=niter, burnin=burnin, samples_filename=samples_filename,
                          appendsamples=appendsamples, return_samples=return_samples,
                          rvdatalabels=rvdatalabels, addnparams=addnparams, verbose=verbose,
                          bin_plot_data=bin_plot_data, plot=plot, include_rm=True, scaled_ball=scaled_ball)


def fitTransitsAndRVsWithMCMC(tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, priors, amp=1e-4, nwalkers=32, niter=100, burnin=20, samples_filename="default_samples.h5", appendsamples=False, return_samples=True, rvlabel="", addnparams=0, verbose=False, plot=False, plot_individual_transits=False, best_fit_from_mode=False, timelabel='BJD', plot_rv_bins=False, rvdatalabels=[], include_rm=False, scaled_ball=True, rm_instrument_indexes=None) :
    
    """Joint fit of planetary transits and radial velocities using MCMC (emcee).
    ...
    return_samples : bool, optional (default: True)
        Return the flattened MCMC samples in the posterior dictionary.
    rvlabel : str, optional (default: "")
        Label of the RV data (for plots).
    addnparams : int, optional (default: 0)
    verbose : bool, optional (default: False)
        Print progress information.
    plot : bool, optional (default: False)
    ...
    timelabel : str, optional (default: 'BJD')
        Label of the time axis (for plots).
    plot_rv_bins : bool, optional (default: False)
    rvdatalabels : list, optional (default: [])
        Labels of the RV datasets (for plots).
    """
    posterior = deepcopy(priors)
    ...
    flare_params = posterior["flare_params"]
    planet_params = posterior["planet_params"]
    instrum_params = posterior["instrum_params"] if "instrum_params" in posterior else None
    ...
    if verbose and include_rm and rmlib.has_rm_parameters(planet_params, 0):
        print(rmlib.rm_ldc_report(planet_params, 0, instrum_params=instrum_params))
    ...
    # Initialize the sampler
    sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob_tr_rv, args = [theta_priors, labels, calib_params, rvcalib_params, flare_params, planet_params, tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, False, include_rm, posterior.get("instrum_params", None), posterior.get("instrument_indexes", None), rm_instrument_indexes], backend=backend)
    ...
    if plot :
        ...
        for planet_index in range(int(posterior["planet_params"]["n_planets"])) :
            plot_rv_perplanet_timeseries(posterior["planet_params"], posterior["rvcalib_params"], rv_times, rvs, rverrs, number_of_free_params=len(posterior["theta"]), planet_index=planet_index, samples=samples, labels=labels, nsamples=100, plot_residuals=True, rvdatalabels=rvdatalabels, phase_plot=True, bindata=plot_rv_bins, binsize=0.05)
...
def calculate_bic_gp (posterior, tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, gp_params, gp_rv, gp_blong, gp_phot, rv_gp_feed, blong_gp_feed, phot_gp_feed, spec_constrained=False, rvnull=False) :
    ...
    blong_gp_feed
    phot_gp_feed
    spec_constrained : bool, optional (default: False)
    rvnull : bool, optional (default: False)
    """
...
def fitTransits_and_RVs_ols(tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, priors, fix_eccentricity=False, fix_tc_for_non_transit=False, flare_post_type="FIXED", flares_unc=0.05, calib_post_type="FIXED", rvcalib_post_type="FIXED", calib_unc=0.05, verbose=False, plot=False, rvlabel="") :
    ...
    """Joint least-squares (OLS) fit of transits and radial velocities, used to refine initial guesses before MCMC.
    ...
    priors
        Priors dictionary (see fitlib/priorslib readers).
    fix_eccentricity : bool, optional (default: False)
    fix_tc_for_non_transit : bool, optional (default: False)
    flare_post_type : str, optional (default: "FIXED")
    flares_unc : float, optional (default: 0.05)
    calib_post_type : str, optional (default: "FIXED")
    rvcalib_post_type : str, optional (default: "FIXED")
    calib_unc : float, optional (default: 0.05)
    verbose : bool, optional (default: False)
        Print progress information.
    plot : bool, optional (default: False)
    ...
    if fix_eccentricity :
        ...
        for i in range(len(labels)) :
            ...
            if ("esinw_" not in labels[i]) and ("ecosw_" not in labels[i]) and ("u0_" not in labels[i]) and ("u1_" not in labels[i]) and ("tc_001" not in labels[i]):
...
def fit_RVs_ols(rv_times, rvs, rverrs, priors, fix_eccentricity=False, rvcalib_post_type="FIXED", calib_unc=0.05, verbose=False, plot=False, rvlabel="") :
    ...
    """Least-squares (OLS) fit of a Keplerian orbit to radial velocity data.
    ...
    if fix_eccentricity :
        ...
        for i in range(len(labels)) :
            #if ("ecc_" not in labels[i]) and ("w_" not in labels[i])  :
            if ("esinw_" not in labels[i]) and ("ecosw_" not in labels[i])  :
                theta_copy = np.append(theta_copy,theta[i])
                labels_copy.append(labels[i])
        print("Free parameters before OLS fit:")
        for i in range(len(theta_copy)) :
...
def plot_rv_timeseries(planets_params, rvcalib_params, bjds, rvs, rverrs, number_of_free_params=0, samples=None, labels=None, nsamples=100, planet_index=0, plot_residuals=False, plot_bin_data=True, phasefold=False, t0=0, rvlabel='') :
    ...
    rverrs
        List of RV uncertainty arrays, one per dataset.
    number_of_free_params : int, optional (default: 0)
    samples : optional (default: None)
    labels : optional (default: None)
        Labels of the free parameters.
    nsamples : int, optional (default: 100)
    planet_index : int, optional (default: 0)
        Index of the planet (0-based).
    plot_residuals : bool, optional (default: False)
    plot_bin_data : bool, optional (default: True)
    phasefold : bool, optional (default: False)
...
def plot_rv_global_timeseries(planet_params, rvcalib_params, bjds, rvs, rverrs, number_of_free_params=0, samples=None, labels=None, nsamples=100, plot_residuals=False, rvdatalabels=[]) :
    ...
    """Plot the global RV time series with the model, calibration and residuals.
...
def plot_rv_perplanet_timeseries(planet_params, rvcalib_params, bjds, rvs, rverrs, T0=0, planet_index=0, number_of_free_params=0, samples=None, labels=None, nsamples=100, plot_residuals=False, rvdatalabels=[], phase_plot=True, bindata=False, binsize=0.05) :
    ...
    """Plot the phase-folded RV time series for one planet with the model and residuals.
    ...
    rvdatalabels : list, optional (default: [])
        Labels of the RV datasets (for plots).
    phase_plot : bool, optional (default: True)
    bindata : bool, optional (default: False)
    binsize : float, optional (default: 0.05)
        Bin size in time units [d].
    """
    legends, fmts, colors = [],[],[]
...
def get_rv_per_planet_model(planet_params, T0, ti, tf, timesampling=0.001, planet_index=0, phasefold=False):
    ...
    for i in range(n_planets) :
        per = planet_params['per_{0:03d}'.format(i)]
        tt = planet_params['tc_{0:03d}'.format(i)]
        esinw, ecosw = exoplanetlib.get_esinw_ecosw(planet_params, i)
        ecc, om = exoplanetlib.get_ecc_omg(esinw, ecosw)
        #ecc = planet_params['ecc_{0:03d}'.format(i)]
        #om = planet_params['w_{0:03d}'.format(i)]
        #tp = planet_params['tp_{0:03d}'.format(planet_index)]
...
def plot_gp_photometry(lc, posterior, gp, y, binsize, phaseplot=False) :
    ...
    binsize
        Bin size in time units [d].
    phaseplot : bool, optional (default: False)
    """
...
def plot_gp_rv(posterior, gp_rv, gp_feed, bjds, rvs, rverrs, samples=None, labels=None, nsamples=100, planet_index=0, default_legend="SPIRou RV", t0=0) :
    ...
    planet_index : int, optional (default: 0)
        Index of the planet (0-based).
    default_legend : str, optional (default: "SPIRou RV")
    t0 : int, optional (default: 0)
    """
    legends, fmts, colors = [],[],[]
    if len(rvs) == 1 :
...
def set_star_rotation_priors(gp_phot, gp_blong, gp_rv, fix_phot_params=False, fix_blong_params=False, fix_rv_params=False, shared_Prot=False, ini_prot=0., period_lim=(4,100), decaytime_lim=(50,1000), fix_phot_decaytime=False, fix_spec_decaytime=False, smoothfactor_lim=(0.25,1.25), fix_phot_smoothfactor=False, fix_spec_smoothfactor=False) :
    ...
    gp_blong
    gp_rv
    fix_phot_params : bool, optional (default: False)
    fix_blong_params : bool, optional (default: False)
    fix_rv_params : bool, optional (default: False)
    shared_Prot : bool, optional (default: False)
    ini_prot : float, optional (default: 0.)
    period_lim : list, optional (default: (4,100))
    decaytime_lim : list, optional (default: (50,1000))
    fix_phot_decaytime : bool, optional (default: False)
    fix_spec_decaytime : bool, optional (default: False)
    smoothfactor_lim : list, optional (default: (0.25,1.25))
    fix_phot_smoothfactor : bool, optional (default: False)
    fix_spec_smoothfactor : bool, optional (default: False)
    """
...
def set_star_rotation_gp(posterior, gp_priors, lc, bjds, rv, rverr, bbjds=None, blong=None, blongerr=None, binsize = 0.1, gp_ls_fit=True, run_gp_mcmc=False, amp=1e-4, nwalkers=32, niter=300, burnin=50, remove_transits=False, plot=False, verbose=False, spec_constrained=False) :

    """Set star rotation gp.
    ...
    bbjds : optional (default: None)
    blong : optional (default: None)
    blongerr : optional (default: None)
    binsize : float, optional (default: 0.1)
        Bin size in time units [d].
    gp_ls_fit : bool, optional (default: True)
    run_gp_mcmc : bool, optional (default: False)
    amp : float, optional (default: 1e-4)
        Amplitude of the initial ball of walkers around the starting point.
    nwalkers : int, optional (default: 32)
        Number of MCMC walkers.
    niter : int, optional (default: 300)
        Number of MCMC steps.
    burnin : int, optional (default: 50)
        Number of MCMC burn-in steps discarded from the chain.
    remove_transits : bool, optional (default: False)
    plot : bool, optional (default: False)
        Show diagnostic plots.
    verbose : bool, optional (default: False)
        Print progress information.
...
def plot_gp_blong(bjds, blong, blongerr, posterior, gp_blong, fold_period, phase_plot=True, ylabel=r"B$_l$ [G]", timesampling=0.001, number_of_free_params=0) :
    ...
    phase_plot : bool, optional (default: True)
    ylabel : str, optional (default: r"B$_l$ [G]")
    timesampling : float, optional (default: 0.001)
    number_of_free_params : int, optional (default: 0)
    """
...
def lnprob_tr_rv_gp(theta, theta_priors, labels, calib_params, rvcalib_params, flare_params, planet_params, tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, gp_params, gp_rv, gp_blong, gp_phot, rv_gp_feed, blong_gp_feed, phot_gp_feed, spec_constrained, rvnull=False):
    ...
    if gp_blong is not None :
        ...
        prob = lp + gp_phot.log_likelihood(phot_gp_feed["y"]) + lnlikelihood_tr_rv_gp(theta, labels, calib_params, rvcalib_params, flare_params, planet_params, tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, gp_rv, gp_phot, rv_gp_feed, phot_gp_feed, rvnull=rvnull)
...
def update_star_rotation_gp_params(params, labels, theta, gp_phot, gp_blong, gp_rv, spec_constrained=False) :
    ...
    """Update star rotation gp params.
...
def fitTransitsAndRVsWithMCMCAndGP(tr_times, fluxes, fluxerrs, rv_times, rvs, rverrs, b_times, blong, blongerr, priors, gp_priors, lc, phot_binsize=0.1, amp=1e-4, nwalkers=32, niter=100, burnin=20, samples_filename="default_samples.h5", appendsamples=False, return_samples=False, verbose=False, plot=False, plot_individual_transits=False, rvlabel="", gp_spec_constrained=False) :
    ...
    """Joint fit of transits, RVs and activity (B-long) data with a quasi-periodic Gaussian Process using MCMC.
    ...
    gp_priors
    lc
    phot_binsize : float, optional (default: 0.1)
    amp : float, optional (default: 1e-4)
        Amplitude of the initial ball of walkers around the starting point.
    nwalkers : int, optional (default: 32)
        Number of MCMC walkers.
    ...
    rvlabel : str, optional (default: "")
        Label of the RV data (for plots).
    gp_spec_constrained : bool, optional (default: False)
    """
    posterior = deepcopy(priors)
    ...
    for i in range(posterior["n_planets"]) :
        ...
        planet_posterior = priorslib.derive_filename(posterior["planet_priors_files"][i], "_posterior.pars")
        if verbose:
        ...
        systemlib.export_system_posterior(posterior["planet_priors_files"][i], planet_posterior, verbose=verbose)
    ...
    if verbose :
        ...
        print("----------------")
    gp_posterior = priorslib.derive_filename(gp_priors["gp_priorsfile"], "_posterior.pars")
    if verbose:
...
def reduce_gp(gp, x, y, yerr, gp_feed, subtract=True) :
    ...
    yerr
        Array of y uncertainties.
    gp_feed
    subtract : bool, optional (default: True)
    """
...
def detrend_RV_data(priors, bjds, rvs, rverrs, xs, xerrs, keep_masks=[], n_sigma_clip=0, epsilon=1.5, x_label="x", plot=False) :
    ...
    xs
    xerrs
    keep_masks : list, optional (default: [])
