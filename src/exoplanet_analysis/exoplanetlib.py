...
def batman_transit_model(time, planet_params, planet_index=0, instrum_params=None, instrum_index=0) :
    ...
    esinw, ecosw = get_esinw_ecosw(planet_params, planet_index)
    params.ecc, params.w = get_ecc_omg(esinw, ecosw)
...
def get_ecc_omg(esinw, ecosw) :
    """Convert (e sin w, e cos w) into eccentricity and argument of periastron.

    Parameters
    ----------
    esinw : float
        Eccentricity times the sine of the argument of periastron.
    ecosw : float
        Eccentricity times the cosine of the argument of periastron.
    ...
    omg : float
        Argument of periastron [deg]. Returns 90 for a circular orbit.

    Notes
    -----
    Before version 1.2.0 this function returned ``arcsin(esinw/ecc)`` in
    radians (losing the quadrant), while all callers use the result in
    degrees (e.g. for the batman transit model). This has been corrected to
    ``atan2(esinw, ecosw)`` in degrees.
    """
    ecc = np.sqrt(esinw*esinw + ecosw*ecosw)
    omg = 90.
    if ecc :
        omg = np.degrees(np.arctan2(esinw, ecosw))
    return ecc, omg


def get_esinw_ecosw(planet_params, planet_index=0) :
    """Return (e sin w, e cos w) from a planet parameters dictionary.

    Supports both the current esinw/ecosw parameterization and the legacy
    ecc/w parameterization (with w in degrees) found in older priors files.
    ...
    -------
    esinw, ecosw : float
        The eccentricity parameterization values.
    """
    es_key = 'esinw_{0:03d}'.format(planet_index)
    ec_key = 'ecosw_{0:03d}'.format(planet_index)
    if es_key in planet_params and ec_key in planet_params :
        return planet_params[es_key], planet_params[ec_key]
    ecc = planet_params.get('ecc_{0:03d}'.format(planet_index), 0.)
    w = planet_params.get('w_{0:03d}'.format(planet_index), 90.)
    wrad = np.deg2rad(w)
    return ecc*np.sin(wrad), ecc*np.cos(wrad)
